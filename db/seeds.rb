...
text_first = 'Well done! You have made a first request. What do you see the status code is? You can find the status code by making the same curl request, but now use the `-i` tag before typing your URL with curl, like so: `curl -i [URL]`. 

We have just fetched the clue called `feeling-lucky`. Now make a request to get the clue `feeling-lucky`. What is its status code? This should be found all the way at the top of the response.

Finally, make a request to get the clue `clue-code-STATUS_CODE`, where STATUS_CODE stands for the found code. This clue should return a 200.'
